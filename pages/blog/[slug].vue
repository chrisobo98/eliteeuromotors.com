  ...
  <section v-if="post" class="relative justify-between lg:flex bg-white dark:bg-[#1d1d1d]">
    <article class="w-10/12 mx-auto lg:w-6/12">
        ...
        <div class="space-y-5">
          <div class="mt-7">
            <div class="overflow-hidden rounded">
              <img :alt="post.title" :src="post.image" class="object-cover rounded h-48 w-full" />
            </div>
          </div>
        </div>
      </section>
      ...
      <div class="mb-2 mt-5 flex max-w-full flex-col border-b pb-3 md:hidden">
        <label class="mb-sm text-base font-semibold" for="toc">Table of contents</label>
        <select
          ...
          id="toc"
        >
          <option v-for="section in post.toc" :key="section.id" :value="`#${section.id}`">{{ section.title }}</option>
        </select>
      </div>
    ...
    <aside class="sticky top-10 space-y-8 mx-8 self-start border-t pb-8 pt-5 lg:mt-0 lg:w-3/12 lg:border-l lg:border-t-0 lg:pl-[4.16%]">
      <div class="flex flex-col gap-y-2">
        <span class="text-sm text-secondary">Written by</span>
        <div class="flex items-center gap-x-1.5">
          <img :alt="post.author" :src="post.authorImage" class="h-10 w-10 self-start rounded-full border" />
          <div class="leading-snug">
            <span class="whitespace-pre text-base font-semibold text-primary">{{ post.author }}</span>
          </div>
        </div>
        <span class="mt-2 text-sm text-secondary">Last updated <time :datetime="post.date" class="font-medium text-primary">{{ formattedDate }}</time></span>
      </div>
    </aside>
  </section>
