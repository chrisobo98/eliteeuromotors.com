          ...
          <div class="tabs lg:w-1/3">
            ...
            >
              <div class="tab-content flex items-center">
                <img class="w-12 h-12 rounded-full" :src="review.profileImg" :alt="review.name" />
                <div class="tab-details ml-4">
                  <div class="name font-semibold text-gray-900 dark:text-white">{{ review.name }}</div>
                  <div class="date text-sm font-light text-gray-500 dark:text-gray-300">{{ review.date }}</div>
                </div>
              </div>
            </div>
          </div>
          <div class="testimonial p-8 border border-gray-200 dark:border-gray-700 rounded-lg flex-grow">
            <div class="flex mb-4 justify-center">
              <svg v-for="n in 5" :key="n" :class="{ 'text-yellow-400': n <= reviews[selectedTab].stars, 'text-gray-300': n > reviews[selectedTab].stars }" class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.378 4.232a1 1 0 00.95.69h4.462c.969 0 1.371 1.24.588 1.81l-3.62 2.632a1 1 0 00-.364 1.118l1.378 4.232c.3.921-.755 1.688-1.54 1.118l-3.62-2.632a1 1 0 00-1.176 0l-3.62 2.632c-.785.57-1.84-.197-1.54-1.118l1.378-4.232a1 1 0 00-.364-1.118L2.98 9.66c-.783-.57-.381-1.81.588-1.81h4.462a1 1 0 00.95-.69l1.378-4.232z"/>
              </svg>
            </div>
            <blockquote class="mx-auto max-w-2xl text-gray-500 dark:text-gray-400">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ reviews[selectedTab].title }}</h3>
              <p v-for="(paragraph, i) in reviews[selectedTab].text.split('\n')" :key="i" class="my-4">{{ paragraph }}</p>
            </blockquote>
            <figcaption class="flex justify-center items-center space-x-3 mt-4">
              <img class="w-9 h-9 rounded-full" :src="reviews[selectedTab].profileImg" :alt="reviews[selectedTab].name">
              <div class="space-y-0.5 font-medium dark:text-white text-left">
                <div>{{ reviews[selectedTab].name }}</div>
                <div class="text-sm font-light text-gray-500 dark:text-gray-400">{{ reviews[selectedTab].position }}</div>
              </div>
            </figcaption>
          </div>
        </div>
        <div class="text-center mt-8">
          <a href="#" class="py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-red-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-red-600 dark:text-white dark:border-red-400 dark:hover:text-white dark:hover:bg-red-700">Show more...</a> 
        </div>
      </div>
    </section>
  </template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const reviews: Review[] = [
        {
          id: '1',
          profileImg: 'https://lh3.googleusercontent.com/a/ACg8ocKHel7VnFnfsSCVbHuFOQQOPHI7pklkgW_EJEhbukD150dP4w=s120-c-rp-mo-br100',
          name: 'Matt Queen',
          date: 'May 27, 2024',
          stars: 5,
        ...
        {
          id: '2',
          profileImg: 'https://lh3.googleusercontent.com/a-/ALV-UjXh1aM4uPM6MBEs7a9cv94eP8pXydmuv9apQB7wpDxIaZUnB34=s120-c-rp-mo-br100',
        ...
        {
          id: '3',
          profileImg: 'https://lh3.googleusercontent.com/a/ACg8ocL-EZG4W6QuZBWcoQPOxj2t9GmEpw3LrdUyIKt06cKmM_St1A=s120-c-rp-mo-br100',
          name: 'Pablo Vegas',
        ...
        {
          id: '4',
          profileImg: 'https://lh3.googleusercontent.com/a/ACg8ocI4aixKEO8SE2ig0pxhfOIIGOloGvRbxWAwEXGIdDX7iBck7A=s120-c-rp-mo-br100',
          name: 'Dennis Zavett',
        ...
        {
          id: '5',
          profileImg: 'https://lh3.googleusercontent.com/a-/ALV-UjVTaw_nDXmG8_mAjbzDxUowDI3gc9cJytt4gRNICMw71rJOYY-3=s120-c-rp-mo-br100',
          name: 'Charles Levin',
